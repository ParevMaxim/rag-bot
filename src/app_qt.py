...
def install_ollama_if_missing():
    """
    Проверяет наличие Ollama. Если нет — скачивает и предлагает установить.
    Возвращает True, если Ollama готова к работе.
    """
    # 1. Проверяем, есть ли команда ollama
    if shutil.which("ollama"):
        return True

    # Если это не Windows, мы не можем автоматически установить (пока)
    if os.name != "nt":
    ...
    # 5. Проверяем снова после установки
    if shutil.which("ollama"):
...
class IndexWorker(QThread):
    progress_signal = Signal(str, int)  # stage, percent
    finished_signal = Signal(object)    # error: None | str
...
class AnswerWorker(QThread):
    finished_signal = Signal(str, object)  # answer, error
...
class DebugWorker(QThread):
    finished_signal = Signal(list, object)  # hits, error
...
class ModelPullWorker(QThread):
    progress_signal = Signal(str)        # текст статуса
    finished_signal = Signal(object)     # error: None | str
    ...
    def run(self):
        try:
            ...
            for model in self.models:
                # pull возвращает стрим объектов {status, completed, total}
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # после инициализации UI — проверяем модели в Ollama
    ...
    def _build_left_panel(self, main_layout: QHBoxLayout):
        ...
        # Тень справа (для красоты)
        # left.setGraphicsEffect(...) - можно добавить тень при желании
    ...
    def check_models_on_start(self):
        """Проверяем, есть ли CHAT_MODEL_MAIN и EMBEDDING_MODEL в Ollama.
        Если нет — автоматически скачиваем.
        """
        ...
        try:
            ...
            # Список моделей может иметь разную структуру в зависимости от версии
            # Обычно client.list() возвращает {'models': [{'name': '...', ...}, ...]}
        ...
        # Важно: у Ollama имена могут быть с тегом (llama3.1:latest).
        # Простая проверка вхождения строки.
        missing = []
        for n in needed:
            # Если точное совпадение не найдено, ищем по началу (напр. 'llama3.1' в 'llama3.1:latest')
            found = False
            for h in have:
        ...
        self.progress_bar.setRange(0, 0)  # индикатор неопределённого прогресса
    ...
    def on_models_finished(self, error: Optional[str]):
        ...
        if error:
            ...
            QMessageBox.critical(
                ...
                "Вы можете попробовать запустить вручную в терминале:\n"
                f'  ollama pull {CHAT_MODEL_MAIN}\n'
    ...
    def append_bot(self, text: str):
        ...
        # Бот - слева, белый фон с рамкой
        bubble = QLabel(text)
        bubble.setWordWrap(True)
        bubble.setTextInteractionFlags(Qt.TextSelectableByMouse)  # Чтобы можно было копировать текст
        bubble.setStyleSheet(
            """
            QLabel {
                background-color:#ffffff;
...
def main():
    # Создаем приложение СНАЧАЛА, чтобы работали QMessageBox
    app = QApplication(sys.argv)
    
    # 1. Проверка и установка Ollama
    if not install_ollama_if_missing():
        sys.exit(0)

    # 2. Гарантируем наличие папок
